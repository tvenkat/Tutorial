 ...
 */
tvenkat_register_event_handler('init', 'system', 'roles_init');
...
function roles_init($event, $type, $object) {
	tvenkat_extend_view('forms/useradd', 'roles/useradd');
	...
	tvenkat_register_plugin_hook_handler('roles:config', 'role', 'roles_get_roles_config');

	// Catch all actions and page route requests
	tvenkat_register_plugin_hook_handler('action', 'all', 'roles_actions_permissions');
	tvenkat_register_plugin_hook_handler('route', 'all', 'roles_pages_permissions');
	...
	tvenkat_register_plugin_hook_handler('register', 'all', 'roles_menus_permissions', 9999);
	
	// Set up roles based hooks and event listener, after all plugin is initialized
	tvenkat_register_event_handler('ready', 'system', 'roles_hooks_permissions');
	tvenkat_register_event_handler('ready', 'system', 'roles_events_permissions');
	tvenkat_register_event_handler('create', 'user', 'roles_create_user');

	// Check for role configuration updates
	if (tvenkat_is_admin_logged_in()) {	// @TODO think through if this should rather be a role-based permission
		run_function_once('roles_update_100_to_101');
		tvenkat_register_event_handler('ready', 'system', 'roles_check_update');
	}

	// Set up roles based view management
	tvenkat_register_event_handler('ready', 'system', 'roles_register_views');
 ...
 *
 * For view extension and replacements the function simply calls the corresponding {@link tvenkat_extend_view()} and
 * {@link tvenkat_set_view_location()} functions, to post-register views after all plugins have been initalized.
 *
 ...
 * there are hundreds of views contributing to any tvenkat page. Listening for all "views", "all" hooks would
 * be quite a waste.
 *
...
function roles_register_views($event, $type, $object) {
	$role = roles_get_role();
	if (tvenkat_instanceof($role, 'object', 'role')) {
		...
		if (is_array($role_perms) && !empty($role_perms)) {
			foreach ($role_perms as $view => $perm_details) {
				switch ($perm_details['rule']) {
					case 'deny':
						tvenkat_register_plugin_hook_handler('view', $view, 'roles_views_permissions');
						break;
					case 'extend':
						...
						tvenkat_extend_view($view, $view_extension, $priority, $viewtype);
						break;
					case 'replace':
						$params = $perm_details['view_replacement'];
						$location = tvenkat_get_root_path() . roles_replace_dynamic_paths($params['location']);
						$viewtype = isset($params['viewtype']) ? $params['viewtype'] : '';
						tvenkat_set_view_location($view, $location, $viewtype);
						break;
					case 'allow':
					default:
						break;
		...
		if (is_array($role_perms) && !empty($role_perms)) {
			foreach ($role_perms as $action => $perm_details) {
				if (roles_path_match(roles_replace_dynamic_paths($action), $type)) {
					switch ($perm_details['rule']) {
						case 'deny':
							register_error(tvenkat_echo('roles:action:denied'));
							return false;
							break;
						case 'allow':
						default:
...
function roles_menus_permissions($hook, $type, $return_value, $params) {
						...
						switch ($perm_details['rule']) {
							...
							case 'extend':
								if ($menu_name === $prepared_menu_name) {
									...
									$menu_obj = TvenkatMenuItem::factory($menu_item);
									tvenkat_register_menu_item($menu_name, $menu_obj);
	...
	// Return the updated menu to the hook triggering function (tvenkat_view_menu)
	return $updated_menu;
}
		...
		if (is_array($role_perms) && !empty($role_perms)) {
			foreach ($role_perms as $page => $perm_details) {
				if (roles_path_match(roles_replace_dynamic_paths($page), $page_path)) {
					switch ($perm_details['rule']) {
						case 'deny':
							register_error(tvenkat_echo('roles:page:denied'));
				...
				switch ($perm_details['rule']) {
					case 'deny':
						...
						if (is_array($params)) {
							...
							tvenkat_unregister_plugin_hook_handler($hook_name, $type, $handler);
						} else {
							global $CONFIG;
					...
					case 'extend':
						...
						tvenkat_register_plugin_hook_handler($hook_name, $type, $handler, $priority);
						break;
					case 'replace':
						$params = $perm_details['hook'];
						...
						$priority = isset($params['priority']) ? $params['priority'] : 500;
						tvenkat_unregister_plugin_hook_handler($hook_name, $type, $old_handler);
						tvenkat_register_plugin_hook_handler($hook_name, $type, $new_handler, $priority);
						break;
					case 'allow':
					default:
						break;
			...
			foreach ($role_perms as $event => $perm_details) {
				...
				switch ($perm_details['rule']) {
					case 'deny':
						...
						if (is_array($params)) {
							$handler = $params['handler'];
							tvenkat_unregister_event_handler($event_name, $type, $handler);
						} else {
							global $CONFIG;
					...
					case 'extend':
						...
						$handler = $params['handler'];
						$priority = isset($params['priority']) ? $params['priority'] : 500;
						tvenkat_register_event_handler($event_name, $type, $handler, $priority);
						break;
					case 'replace':
						$params = $perm_details['hook'];
						$old_handler = $params['old_handler'];
						$new_handler = $params['new_handler'];
						$priority = isset($params['priority']) ? $params['priority'] : 500;
						tvenkat_unregister_event_handler($event_name, $type, $old_handler);
						tvenkat_register_event_handler($event_name, $type, $new_handler, $priority);
						break;
					case 'allow':
					default:
						break;
...
function roles_user_settings_save($hook_name, $entity_type, $return_value, $params) {
	...
	if ($res === false) {
		register_error(tvenkat_echo('user:role:fail'));
		return false;
	} else if ($res === true) {
		system_message(tvenkat_echo('user:role:success'));
		return true;
	}
...
function roles_create_user($event, $type, $user) {
	...
	if (tvenkat_is_admin_logged_in() && $rolename) {
