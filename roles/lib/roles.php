 ...
 *
 * @param TvenkatUser $user

 * @return TvenkatRole The role the user belongs to
 */
function roles_get_role($user = null) {

	$user = $user ? $user : tvenkat_get_logged_in_user_entity();
	...
	if (tvenkat_instanceof($user, 'user')) {
		...
		$roles = tvenkat_get_entities_from_relationship($options);
 ...
 *
 * @param TvenkatRole $role The role to be assigned
 * @param TvenkatUser $user The user the role needs to be assigned to
 ...
 *
 * @return mixed An array of TvenkatRole objects defined in the system, or false if none found
 */
...
function roles_get_all_roles() {
	...
	return tvenkat_get_entities($options);
 ...
 * the list of selectable roles - as default roles are automatically assigned to users based on their Tvenkat membership type
 *
 * @return mixed An array of non-default TvenkatRole objects defined in the system, or false if none found
 */
function roles_get_all_selectable_roles() {

	$dbprefix = tvenkat_get_config('dbprefix');
	$reserved_role_names = "('" . implode("','", TvenkatRole::getReservedRoleNames()) ."')";
	$options = array(
		'type' => 'object',
		'subtype' => 'role',
		'limit' => 0,
 ...
 * @param TvenkatRole $role The role to check for permissions
 ...
 *
 * @global array $PERMISSIONS_CACHE In-memory cache for role permission
 * @param TvenkatRole $role The role to cache permissions for
 */
 ...
 * @return mixed An TvenkatRole object if it could be found based on the name, false otherwise
 */
...
function roles_get_role_by_name($role_name) {
	...
	$role_array = tvenkat_get_entities_from_metadata($options);
...
function roles_filter_role_name($role_name, $user_guid = null) {
	...
	if ($user_guid) {
		if (tvenkat_is_admin_user((int)$user_guid)) {
			return ADMIN_ROLE;
		} else {
			return DEFAULT_ROLE;
		}
	} else if (!tvenkat_is_logged_in()) {
		return VISITOR_ROLE;
	} else if (tvenkat_is_admin_logged_in()) {
		return ADMIN_ROLE;
	} else {
		return DEFAULT_ROLE;
	}
...
/**
 * Processes the configuration files and generates the appropriate TvenkatRole objects.
 *
...
function roles_create_from_config($roles_array) {
	...
	$roles = tvenkat_get_entities($options);
	...
	foreach($roles_array as $rname => $rdetails) {
		...
		if (tvenkat_instanceof($current_role, 'object', 'role')) {
			// Update existing role obejct
			$current_role->title = tvenkat_echo($rdetails['title']);
			$current_role->extends = $rdetails['extends'];
			$current_role->permissions = serialize($rdetails['permissions']);
			$current_role->save();
		} else {
			// Create new role object
			$new_role = new TvenkatRole();
			$new_role->title = tvenkat_echo($rdetails['title']);
			$new_role->owner_guid = tvenkat_get_logged_in_user_guid();
...
function roles_check_update() {
	$hash = tvenkat_get_plugin_setting('roles_hash', 'roles');
	...
	if ($hash != $current_hash) {
		...
		tvenkat_set_plugin_setting('roles_hash', $current_hash, 'roles');
	}
}
 ...
 * @param TvenkatMenuItem $menu_obj The replacement menu item
 * 
 ...
 *
 * @return array The array of TvenkatMenuItem objects from the menu
 */
...
function roles_replace_dynamic_paths($str) {
	$res = $str;
	$user = tvenkat_get_logged_in_user_entity();
	if (tvenkat_instanceof($user, 'user')) {
	...
	$pageowner_guid = tvenkat_trigger_plugin_hook('page_owner', 'system', NULL, 0);
	$pageowner = get_entity($pageowner_guid);
	
	if (tvenkat_instanceof($pageowner, 'user')) {
...
function roles_check_context($permission_details, $strict = false) {
	...
	if (is_array($permission_details['context'])) {
		if ($strict) {
			$result = in_array(tvenkat_get_context(), $permission_details['context']);
		} else {
