...
tvenkat_register_event_handler('init', 'system', 'roles_acl_init');

function roles_acl_init() {

	tvenkat_register_library('roles.acl', tvenkat_get_plugins_path() . 'roles_acl/lib/acl.php');
	tvenkat_load_library('roles.acl');

	run_function_once('roles_acl_setup_role_based_acls');

	tvenkat_register_admin_menu_item('administer', 'acl', 'roles');

	tvenkat_register_action('roles_acl/settings', tvenkat_get_plugins_path() . 'roles_acl/actions/roles_acl/settings.php', 'admin');

	// look at the role acl settings and return collections that match the criteria
	tvenkat_register_plugin_hook_handler('access:collections:write', 'user', 'roles_acl_write_access_array');
	...
	tvenkat_register_event_handler('create', 'metadata', 'roles_acl_create_role_object');
	tvenkat_register_event_handler('delete', 'object', 'roles_acl_delete_role_object');

	// Listen to change in users' roles and update their ACL membership
	tvenkat_register_event_handler('all', 'has_role', 'roles_acl_role_change');

	// Listen to changes in friendship to update user's ACL membership
	tvenkat_register_event_handler('all', 'friend', 'roles_acl_friendship_status');
}
...
function roles_acl_create_role_object($event, $type, $obj) {
	...
	if (!tvenkat_instanceof($entity, 'object', 'role')) {
		return true;
	}
...
function roles_acl_write_access_array($hook, $type, $return, $params) {
	...
	$user_guid = tvenkat_extract('user_id', $params);
	...
	if (isset($acls['global'][$role->name])) {
		foreach ($acls['global'][$role->name] as $acl_member_role) {
			...
			if ($collection) {
				$custom["$collection->id"] = tvenkat_echo($collection->name);
			}
		}
	}
